...
public interface RespostaRepository extends JpaRepository<Resposta, Long> {
    ...
    @Query(value = """
        ...
        JOIN questao q ON r.questao_id = q.id
        JOIN questao_prova_secao qps_c ON q.id = qps_c.questao_id JOIN prova_secao ps_c ON qps_c.prova_secao_id = ps_c.id JOIN prova p_c ON ps_c.prova_id = p_c.id JOIN concurso c ON p_c.concurso_id = c.id
        JOIN alternativa a ON r.alternativa_id = a.id
    ...
    @Query("SELECT CAST(cc.cargo.nivel AS string), COUNT(DISTINCT r.questao.id), SUM(CASE WHEN a.correta = true THEN 1 ELSE 0 END) FROM Resposta r JOIN r.questao q JOIN r.alternativaEscolhida a JOIN q.secoes qs_c JOIN qs_c.provaSecao ps_c JOIN ps_c.prova p_c JOIN p_c.concurso c JOIN c.concursoCargos cc WHERE c.banca.id = :bancaId AND q.anulada = false GROUP BY cc.cargo.nivel")
    List<Object[]> countRespondidasAcertadasByBancaIdGroupByNivel(@Param("bancaId") Long bancaId);

    @Query("SELECT c.instituicao.area, COUNT(DISTINCT r.questao.id), SUM(CASE WHEN a.correta = true THEN 1 ELSE 0 END) FROM Resposta r JOIN r.questao q JOIN r.alternativaEscolhida a JOIN q.secoes qs_c JOIN qs_c.provaSecao ps_c JOIN ps_c.prova p_c JOIN p_c.concurso c WHERE c.banca.id = :bancaId AND q.anulada = false GROUP BY c.instituicao.area")
    List<Object[]> countRespondidasAcertadasByBancaIdGroupByAreaInstituicao(@Param("bancaId") Long bancaId);

    @Query("SELECT cc.cargo.area, COUNT(DISTINCT r.questao.id), SUM(CASE WHEN a.correta = true THEN 1 ELSE 0 END) FROM Resposta r JOIN r.questao q JOIN r.alternativaEscolhida a JOIN q.secoes qs_c JOIN qs_c.provaSecao ps_c JOIN ps_c.prova p_c JOIN p_c.concurso c JOIN c.concursoCargos cc WHERE c.banca.id = :bancaId AND q.anulada = false GROUP BY cc.cargo.area")
    List<Object[]> countRespondidasAcertadasByBancaIdGroupByAreaCargo(@Param("bancaId") Long bancaId);

    @Query("SELECT CAST(cc.cargo.nivel AS string), AVG(r.tempoRespostaSegundos) FROM Resposta r JOIN r.questao q JOIN q.secoes qs_c JOIN qs_c.provaSecao ps_c JOIN ps_c.prova p_c JOIN p_c.concurso c JOIN c.concursoCargos cc WHERE c.banca.id = :bancaId AND r.tempoRespostaSegundos IS NOT NULL GROUP BY cc.cargo.nivel")
    List<Object[]> avgTempoByBancaIdGroupByNivel(@Param("bancaId") Long bancaId);

    @Query("SELECT c.instituicao.area, AVG(r.tempoRespostaSegundos) FROM Resposta r JOIN r.questao q JOIN q.secoes qs_c JOIN qs_c.provaSecao ps_c JOIN ps_c.prova p_c JOIN p_c.concurso c WHERE c.banca.id = :bancaId AND r.tempoRespostaSegundos IS NOT NULL GROUP BY c.instituicao.area")
    List<Object[]> avgTempoByBancaIdGroupByAreaInstituicao(@Param("bancaId") Long bancaId);

    @Query("SELECT cc.cargo.area, AVG(r.tempoRespostaSegundos) FROM Resposta r JOIN r.questao q JOIN q.secoes qs_c JOIN qs_c.provaSecao ps_c JOIN ps_c.prova p_c JOIN p_c.concurso c JOIN c.concursoCargos cc WHERE c.banca.id = :bancaId AND r.tempoRespostaSegundos IS NOT NULL GROUP BY cc.cargo.area")
    List<Object[]> avgTempoByBancaIdGroupByAreaCargo(@Param("bancaId") Long bancaId);

    // --- Granular breakdown queries for Instituicao ---
    @Query("SELECT CAST(cc.cargo.nivel AS string), COUNT(DISTINCT r.questao.id), SUM(CASE WHEN a.correta = true THEN 1 ELSE 0 END) FROM Resposta r JOIN r.questao q JOIN r.alternativaEscolhida a JOIN q.secoes qs_c JOIN qs_c.provaSecao ps_c JOIN ps_c.prova p_c JOIN p_c.concurso c JOIN c.concursoCargos cc WHERE c.instituicao.id = :instituicaoId AND q.anulada = false GROUP BY cc.cargo.nivel")
    List<Object[]> countRespondidasAcertadasByInstituicaoIdGroupByNivel(@Param("instituicaoId") Long instituicaoId);

    @Query("SELECT c.banca.id, COUNT(DISTINCT r.questao.id), SUM(CASE WHEN a.correta = true THEN 1 ELSE 0 END) FROM Resposta r JOIN r.questao q JOIN r.alternativaEscolhida a JOIN q.secoes qs_c JOIN qs_c.provaSecao ps_c JOIN ps_c.prova p_c JOIN p_c.concurso c WHERE c.instituicao.id = :instituicaoId AND q.anulada = false GROUP BY c.banca.id")
    List<Object[]> countRespondidasAcertadasByInstituicaoIdGroupByBanca(@Param("instituicaoId") Long instituicaoId);

    @Query("SELECT cc.cargo.id, COUNT(DISTINCT r.questao.id), SUM(CASE WHEN a.correta = true THEN 1 ELSE 0 END) FROM Resposta r JOIN r.questao q JOIN r.alternativaEscolhida a JOIN q.secoes qs_c JOIN qs_c.provaSecao ps_c JOIN ps_c.prova p_c JOIN p_c.concurso c JOIN c.concursoCargos cc WHERE c.instituicao.id = :instituicaoId AND q.anulada = false GROUP BY cc.cargo.id")
    List<Object[]> countRespondidasAcertadasByInstituicaoIdGroupByCargo(@Param("instituicaoId") Long instituicaoId);

    @Query("SELECT cc.cargo.area, COUNT(DISTINCT r.questao.id), SUM(CASE WHEN a.correta = true THEN 1 ELSE 0 END) FROM Resposta r JOIN r.questao q JOIN r.alternativaEscolhida a JOIN q.secoes qs_c JOIN qs_c.provaSecao ps_c JOIN ps_c.prova p_c JOIN p_c.concurso c JOIN c.concursoCargos cc WHERE c.instituicao.id = :instituicaoId AND q.anulada = false GROUP BY cc.cargo.area")
    List<Object[]> countRespondidasAcertadasByInstituicaoIdGroupByAreaCargo(@Param("instituicaoId") Long instituicaoId);

    @Query("SELECT CAST(cc.cargo.nivel AS string), AVG(r.tempoRespostaSegundos) FROM Resposta r JOIN r.questao q JOIN q.secoes qs_c JOIN qs_c.provaSecao ps_c JOIN ps_c.prova p_c JOIN p_c.concurso c JOIN c.concursoCargos cc WHERE c.instituicao.id = :instituicaoId AND r.tempoRespostaSegundos IS NOT NULL GROUP BY cc.cargo.nivel")
    List<Object[]> avgTempoByInstituicaoIdGroupByNivel(@Param("instituicaoId") Long instituicaoId);

    @Query("SELECT c.banca.id, AVG(r.tempoRespostaSegundos) FROM Resposta r JOIN r.questao q JOIN q.secoes qs_c JOIN qs_c.provaSecao ps_c JOIN ps_c.prova p_c JOIN p_c.concurso c WHERE c.instituicao.id = :instituicaoId AND r.tempoRespostaSegundos IS NOT NULL GROUP BY c.banca.id")
    List<Object[]> avgTempoByInstituicaoIdGroupByBanca(@Param("instituicaoId") Long instituicaoId);

    @Query("SELECT cc.cargo.id, AVG(r.tempoRespostaSegundos) FROM Resposta r JOIN r.questao q JOIN q.secoes qs_c JOIN qs_c.provaSecao ps_c JOIN ps_c.prova p_c JOIN p_c.concurso c JOIN c.concursoCargos cc WHERE c.instituicao.id = :instituicaoId AND r.tempoRespostaSegundos IS NOT NULL GROUP BY cc.cargo.id")
    List<Object[]> avgTempoByInstituicaoIdGroupByCargo(@Param("instituicaoId") Long instituicaoId);

    @Query("SELECT cc.cargo.area, AVG(r.tempoRespostaSegundos) FROM Resposta r JOIN r.questao q JOIN q.secoes qs_c JOIN qs_c.provaSecao ps_c JOIN ps_c.prova p_c JOIN p_c.concurso c JOIN c.concursoCargos cc WHERE c.instituicao.id = :instituicaoId AND r.tempoRespostaSegundos IS NOT NULL GROUP BY cc.cargo.area")
